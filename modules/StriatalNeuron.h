    ...
    public:
        ...
        virtual void step(){
            ...
            if(up_down_){ // up-state
                ...
                if( (upstate_<0.05)  || (upstate_ < 0.5 && sum("dopa") < threshold_dopa_)  ){ // transition to the down-state
                    up_down_=false;
                    upstate_=0.0;
                }  
            }
            ...
            mp_+= dt_ /tau_ * (-mp_ + vmpfc_ + bla_ -  sum("inh") + 0.5* sum("dopa") + baseline_ + noise_*(2.0*rand_num-1.0));
